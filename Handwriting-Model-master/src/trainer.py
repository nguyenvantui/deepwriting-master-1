...
class Trainer(object):
    ...
    def train_epoch(self):
        ...
        for batch_index, (data) in enumerate(self.trainloader, 1):
            ...
            # print(onehot)
            # bp()
            # print(strokes[:, strokes.size(1)-1:strokes.size(1), :])
            for idx in range(strokes.size(1)-1):
                input_strokes = strokes[:, idx:idx+1, :]
                output, _ = self.model(input_strokes, onehot)
                # Loss Computation
                if loss is None:
            ...
            inf = float("inf")
            if loss.item() == inf or loss.item() == -inf:
                print("Warning, received inf loss. Skipping it")
            elif loss.item() != loss.item():
